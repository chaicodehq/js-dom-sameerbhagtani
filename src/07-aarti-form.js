...
export function setupAartiForm(formElement, onSuccess, onError) {
    ...
    function handler(e) {
        ...
        const validatedAartiType = validateAartiType(aartiType);

        if (
            validatedName.valid &&
            validatedDate.valid &&
            validatedAartiType.valid
        ) {
        ...
        if (!validateName.valid) arr.push(validateName.error);
        if (!validatedDate.valid) arr.push(validateDate.error);
        if (!validatedAartiType.valid) arr.push(validateAartiType);
